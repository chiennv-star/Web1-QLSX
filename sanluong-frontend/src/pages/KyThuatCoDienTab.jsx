...
export default function KyThuatCoDienTab() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { data: res } = await api.get('/ky-thuat/co-dien')
      setData(res)
    } catch { message.error('Tải dữ liệu Cơ điện thất bại') }
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(r =>
      [r.moTa, r.thietBi, r.khuVuc, r.nguyenNhan, r.bienPhap, r.ketQua, r.nguoiPhuTrach]
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editRecord) {
        const { data: updated } = await api.put(`/ky-thuat/co-dien/${editRecord.id}`, payload)
        setData(prev => prev.map(r => r.id === editRecord.id ? updated : r))
        message.success('Cập nhật thành công')
      } else {
        const { data: created } = await api.post('/ky-thuat/co-dien', payload)
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/ky-thuat/co-dien/${id}`)
  ...
  const columns = [
    ...
    { title: 'Khu vực', dataIndex: 'khuVuc', width: 140 },
    ...
    <div>
      <Space style={{ marginBottom: 12, width: '100%', justifyContent: 'space-between' }} wrap>
        <Input
          ...
          placeholder="Tìm theo mô tả, thiết bị, khu vực, nguyên nhân…"
          value={keyword}
      ...
      <Modal
        open={modalOpen}
        title={editRecord ? 'Sửa nhật ký Cơ điện' : 'Thêm nhật ký Cơ điện'}
        onOk={handleSave}
      ...
      >
        <Form form={form} layout="vertical">
          <Row gutter={12}>
            ...
            <Col span={8}>
              <Form.Item label="Khu vực" name="khuVuc"><Input /></Form.Item>
            </Col>
          </Row>
          ...
          <Row gutter={12}>
            ...
            <Col span={8}>
              <Form.Item label="Mức độ" name="mucDo">
                <Select allowClear options={MUC_DO_OPTIONS.map(o => ({ value: o.value, label: o.value }))} />
              </Form.Item>
            </Col>
            <Col span={8}>
          ...
          <Row gutter={12}>
            <Col span={12}>
              <Form.Item label="Linh kiện thay thế" name="linhKien"><Input /></Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label="Người phụ trách" name="nguoiPhuTrach"><Input /></Form.Item>
            </Col>
          </Row>
          <Form.Item label="Kết quả" name="ketQua"><TextArea rows={2} /></Form.Item>
        </Form>
      </Modal>
    </div>
  )
