...
export default function KyThuatKyThuatTab() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { data: res } = await api.get('/ky-thuat/ky-thuat')
      setData(res)
    } catch { message.error('Tải dữ liệu Kỹ thuật thất bại') }
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(r =>
      [r.moTa, r.thietBi, r.noiDung, r.ghiChu, r.nguyenNhan, r.bienPhap, r.nguoiPhuTrach]
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editRecord) {
        const { data: updated } = await api.put(`/ky-thuat/ky-thuat/${editRecord.id}`, payload)
        setData(prev => prev.map(r => r.id === editRecord.id ? updated : r))
        message.success('Cập nhật thành công')
      } else {
        const { data: created } = await api.post('/ky-thuat/ky-thuat', payload)
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/ky-thuat/ky-thuat/${id}`)
  ...
  return (
    <div>
      ...
      <Modal
        open={modalOpen}
        title={editRecord ? 'Sửa nhật ký Kỹ thuật' : 'Thêm nhật ký Kỹ thuật'}
        onOk={handleSave}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="Nội dung thẩm định / thực hiện" name="noiDung"><TextArea rows={3} /></Form.Item>
          <Form.Item label="Vấn đề gặp phải / giải pháp" name="ghiChu"><TextArea rows={2} /></Form.Item>
